...
The problem specifies that carbohydrate is related to age, relative weight, and protein without bias. For the purpose of this problem, we will assume a linear relationship. We do, however, include an intercept for proper modeling.
...
The analysis was implemented in Python using NumPy for numerical computations. Both OLS and Ridge solutions were computed using the closed-form matrix equations.
...
\begin{table}[h]
    ...
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Metric} & \textbf{OLS} & \textbf{Ridge $\lambda$=1} & \textbf{$\lambda$=2} & \textbf{$\lambda$=100} & \textbf{$\lambda$=600} \\
        \midrule
